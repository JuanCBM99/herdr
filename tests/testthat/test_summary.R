...
test_that("generate_impact_assessment consolidates all modules and applies filters", {
  ...
  res <- testthat::with_mocked_bindings(
    generate_impact_assessment(
      ...
      saveoutput = FALSE,
      region = "North" # Probamos el filtro
    ),
  ...
  # Verificamos consolidación de columnas
  expect_true(all(c("CH4_enteric_Mg", "CH4_manure_Mg", "N2O_direct_kg", "Land_m2") %in% colnames(res)))

  # Verificamos valores sumados (summarise interno de la función)
  ...
  # Probamos que si filtramos por algo inexistente devuelva 0 filas
