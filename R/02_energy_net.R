...
calculate_NEl <- function(saveoutput = TRUE) {
  ...
  results <- categories %>%
    ...
    dplyr::mutate(
      milk_yield = tidyr::replace_na(suppressWarnings(as.numeric(milk_yield)), 0),
      fat_content = tidyr::replace_na(suppressWarnings(as.numeric(fat_content)), 0),
      NEl = (0.0929 * fat_content + 0.0547) * milk_yield / 365
    ) %>%
