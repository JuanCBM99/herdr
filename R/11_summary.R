...
generate_impact_assessment <- function(automatic_cycle = FALSE,
  ...
  # 1. Pipeline calls and unit standardization
  ...
  # Units: CH4_enteric_Mg and CH4_manure_Mg are in Mg (tonnes)
  # N2O values are in kg; convert to Mg (* 1e-3) before applying GWP
  final_summary <- final_summary %>%
    dplyr::mutate(
      CO2eq_enteric  = CH4_enteric_Mg * 28,
      CO2eq_manure   = CH4_manure_Mg * 28,
      CO2eq_n2o      = (N2O_direct_kg + N2O_vol_kg + N2O_lea_kg) * 265 / 1000,
      CO2eq_Total_Gg = (CO2eq_enteric + CO2eq_manure + CO2eq_n2o) / 1000,

      # Intensity: kg CO2eq per m2
      # (CO2eq_Total_Gg * 1e6 converts Gg to kg)
