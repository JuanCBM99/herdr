...
- **The 100% Rule**: These four must sum to **100.0** (e.g., 70 Forage +
  30 Concentrate).
...
- **The 100% Rule**: Ingredients inside each category (like “Forage”)
  must sum to **100.0**.
- **Example:** If your “Forage” macro is 70, your ingredient list for
  Forage (e.g., 50.0 Alfalfa + 50.0 Maize Silage) must sum to 100.0.
...
#### 1. Defining Animal Traits (`livestock_definitions.csv` & `monogastric_definitions.csv`)

For every `animal_tag` defined in your census, you must assign its
biological parameters and associated diet.

- **`diet_tag`**: Links each animal category to one of the diets created
  in Phase 2.

- **Ruminants (`livestock_definitions.csv`)**: Select the appropriate
  **IPCC description** from `ipcc_coefficients.csv`.

  - *How it works:* The selected description (e.g.,
    `cattle and buffalo`) allows the model to automatically retrieve the
    corresponding Tier 2 coefficients ($`C_a`$, $`C_{fi}`$, etc.)
    required for the energy equations. For sheep and goats, the
    pregnancy coefficient ($`C_{pregnancy}`$) is calculated
    automatically from the pregnancy rate and the proportion of single
    and twin births.

- **Monogastrics (`monogastric_definitions.csv`)**: Fill in the
  species-specific biological parameters required by the FEDNA
  equations. Default values are provided for all coefficients, but they
  can be modified if more representative values are available.

#### 2. Body Metrics (`livestock_weights.csv`)

You must replicate the exact combinations of `animal_tag`, `region`,
`subregion`, and `class_flex` used in your census and assign the
corresponding body parameters.

- **`adult_weight`**: Mature body weight of the adult animal (kg).
- **`initial_weight`**: Live weight of the animal at the beginning of
  the production period (kg).
- **`final_weight`**: Live weight of the animal at the end of the
  production period (kg).
- **`productive_period`**: Duration of the production period (days).

##### Additional parameters for breeding sows

The following variables are only required for breeding sows and are used
to estimate gestation, lactation, and nitrogen retention:

- **`sows_gestation_days`**: Gestation length (days).
- **`sows_lactation_days`**: Lactation length (days).
- **`piglet_birth_weight_kg`**: Average birth weight of piglets (kg).
- **`piglet_weaning_weight_kg`**: Average weaning weight of piglets
  (kg).
- **`sow_reserve_gain_kg`**: Body reserve gain of the sow during
  gestation (kg).

💡 **How to Proceed**

1.  **Tag Consistency:** Start by copying the `animal_tag`, `region`,
    `subregion`, and `class_flex` columns from your
    `livestock_census.csv`. They must be identical.
2.  **Add New Tags:** If you created custom animal categories in Phase
    1, include them here as well.
3.  **Assign Body Metrics:** Complete the weight and production-period
    variables for every animal category. If applicable, also fill in the
    sow-specific parameters.
4.  **Assign Diets:** Select the appropriate `diet_tag` for each
    category in `livestock_definitions.csv` or
    `monogastric_definitions.csv`.
5.  **Select IPCC Coefficients:** Open `ipcc_coefficients.csv`, identify
    the description that best matches your animal category, and copy
    that exact value into `livestock_definitions.csv`.
