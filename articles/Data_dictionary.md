...
| `de`                                       | Digestible energy (%). Key for Methane ($Y_{m}$) calculation. |
...
| `animal_category`               | Category of the animal for baseline Methane Production ($B_{0}$). |
...
| `frac_gas_ms`       | Fraction of nitrogen lost as volatilized gases ($NH_{3}$ + $NO_{x}$). |
...
| `coefficient` | Coefficient tag (e.g., `cfi`, $B_{0}$, $Y_{m}$).                |
