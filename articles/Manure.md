...
1.  **Complex Systems:** You **must** ensure consistency with the
    `climate_zone`.
    - If `zone_wet`, `zone_moist` or `zone_montane` is selected → Use
      `wet`.
    - If `zone_dry` is selected → Use `dry`.
2.  **Standard Systems:** Since these have no `climate_zone`, use
...
Use these definitions to determine the correct `climate_zone` based on
local data.
...
Based on MAT and the ratio of Potential Evapotranspiration (PET) to
Precipitation (P).
