---
title: "General Workflow: Step-by-Step Guide"
output: 
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{General Workflow: Step-by-Step Guide}
...
Name your folder (e.g., "my_livestock_study"). 
Working inside an .Rproj is essential; it ensures herdr knows exactly where to find your data and keeps your workspace organized.
...
### 4. Fill the CSVs
Go to the `user_data/` folder in your project directory. Fill the blank templates following the Phases 1-4 detailed below.
To test the package immediately, copy the files from `Examples/Easy_Cattle/` into `user_data/` and skip to step 6.

#### 📂 Folder Structure & File Roles

To keep your project organized, *herdr* separates your project data from internal scientific libraries. To see which specific files you should edit and which ones to leave as references, check the [Technical Reference](Technical_reference.html).
...
* **The 100% Rule**: These four must sum to **100.0** (e.g., 70 Forage + 30 Concentrate).

### 2. Micro Ingredients (`diet_ingredients.csv`)
List exactly what is inside each diet_tag.

* **The 100% Rule**: Ingredients inside each category (like "Forage") must sum to **100.0**. 
* **Example:** If your "Forage" macro is 70, your ingredient list for Forage (e.g., 50.0 Alfalfa + 50.0 Maize Silage) must sum to 100.0.
...
For every `animal_tag` defined in your census, you must assign its biological parameters and associated diet.

* **`diet_tag`**: Links each animal category to one of the diets created in Phase 2.

* **Ruminants (`livestock_definitions.csv`)**: Select the appropriate **IPCC description** from `ipcc_coefficients.csv`.
  * *How it works:* The selected description (e.g., `cattle and buffalo`) allows the model to automatically retrieve the corresponding Tier 2 coefficients ($C_a$, $C_{fi}$, etc.) required for the energy equations. For sheep and goats, the pregnancy coefficient ($C_{pregnancy}$) is calculated automatically from the pregnancy rate and the proportion of single and twin births.

* **Monogastrics (`monogastric_definitions.csv`)**: Fill in the species-specific biological parameters required by the FEDNA equations. Default values are provided for all coefficients, but they can be modified if more representative values are available.

### 2. Body Metrics (`livestock_weights.csv`)

You must replicate the exact combinations of `animal_tag`, `region`, `subregion`, and `class_flex` used in your census and assign the corresponding body parameters.

* **`adult_weight`**: Mature body weight of the adult animal (kg).
* **`initial_weight`**: Live weight of the animal at the beginning of the production period (kg).
* **`final_weight`**: Live weight of the animal at the end of the production period (kg).
* **`productive_period`**: Duration of the production period (days).

#### Additional parameters for breeding sows

The following variables are only required for breeding sows and are used to estimate gestation, lactation, and nitrogen retention:
...
* **`piglet_weaning_weight_kg`**: Average weaning weight of piglets (kg).
* **`sow_reserve_gain_kg`**: Body reserve gain of the sow during gestation (kg).

💡 **How to Proceed**

1. **Tag Consistency:** Start by copying the `animal_tag`, `region`, `subregion`, and `class_flex` columns from your `livestock_census.csv`. They must be identical.
2. **Add New Tags:** If you created custom animal categories in Phase 1, include them here as well.
3. **Assign Body Metrics:** Complete the weight and production-period variables for every animal category. If applicable, also fill in the sow-specific parameters.
4. **Assign Diets:** Select the appropriate `diet_tag` for each category in `livestock_definitions.csv` or `monogastric_definitions.csv`.
5. **Select IPCC Coefficients:** Open `ipcc_coefficients.csv`, identify the description that best matches your animal category, and copy that exact value into `livestock_definitions.csv`.
...
* **Methane ($CH_4$):** Driven by the conditions of the storage (determined by system type, temperature, and duration).
* **Nitrous Oxide ($N_2O$):** Driven by the Nitrogen excretion ($N_{ex}$) and the specific emission factors of each system.
...
1.  **Consult the Reference Guide:** Before entering data, review the **[Manure System Guide](Manure.html)**. You must use the exact system names and variants listed there, as they are the only ones recognized by the IPCC reference database.
