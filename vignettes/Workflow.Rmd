---
title: "Workflow Guide: Step-by-Step"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Workflow Guide: Step-by-Step}
...
This guide explains the workflow for using the `AnimalGEILU` model, from exploring the default reference datasets to introducing and analysing your own Tier 2 custom scenarios.

<img src="images/workflow.png" alt="" style="width:100%; height:auto;">

---

## 1. Preliminary Analysis: Default National Data

**AnimalGEILU** includes default datasets based on national averages and emission factors for **Spain**. These allow you to run the model immediately and obtain a **baseline environmental footprint**.

### Step 1: Run a Quick Analysis

To view the results computed from the internal reference data, simply load the package and run the main function `generate_impact_assessment()`.

```{r run-default-model, eval=FALSE}
# Load the package
library(AnimalGEILU)
...
```{r download_templates, eval=FALSE}
