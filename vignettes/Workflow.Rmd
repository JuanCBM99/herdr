...
The `herdr` package follows a modular logic where information flows from population and nutrition to biological metabolism and, finally, to environmental impact. All user-defined data must be stored in the `user_data/` directory.
...
2. **`crop_yields.csv`**: Enter the yield (kg/ha) and nitrogen data. This tells the model how it affects the **land**.

---

### 💡 Simple Workflow
* **Browse**: Check `feed_characteristics.csv` for existing ingredients.
* **Add**: If missing, add the name and data to **both** `feed_characteristics.csv` and `crop_yields.csv`.
...
* **`average_weight`**: Average weight of . Usually calculated as: $(W_{initial} + W_{final}) / 2$.
* **`weight_gain`**: Daily weight increase. Calculated as: $(W_{final} - W_{initial}) / \text{productive period}$.
...
4.  **Assign Systems and Climates:** For each row, select the appropriate `system_base`, `system_variant`, and `climate_zone` following the logic provided in the System Guide.
