---
title: "Dairy Cattle Spain: North vs South"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Dairy Cattle Spain: North vs South}
...
* **Reference Files (Expert use only):** `feed_characteristics.csv`, `crop_yields.csv`, `ipcc_coefficients.csv`, `ipcc_mm.csv`.
...
Note that the `forage_share` is higher in the North (55%) compared to the South (50%).
...
For each category (forage/concentrate), the `ingredient_share` must sum to **100%**. 
...
Once your CSVs are updated in the `user_data/` folder, run the analysis. The package will automatically perform biological validations (like checking if your cow is eating more than 5.5% of its body weight).
