---
title: "Using the herdr App: A Step-by-Step Workflow"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Using the herdr App: A Step-by-Step Workflow}
...
`herdr_init()` only needs to run once per project — it copies over the
bundled example datasets and creates the `user_data/` folder the app
reads from and saves to. `run_app()` opens the app in your default
browser.
...
- **Upload CSV Files** — if you already have herdr-formatted CSVs, fully
  ...
  model. You'd still open a tab to fix something (a typo, a red cell)
  if the app flags it, but there's no data entry left to do.
...
- **Farm Country / Area** — the country used as the reference for FAO
  trade and production data. This matters most if any ingredient in
  your diet tables has an unknown origin: herdr will estimate it using
  this country's FAO statistics.
...
If you started blank, this is where you enter everything by hand. If
you loaded the example data, this is where you'd tweak it to match your
own herd. And if you uploaded already-completed CSVs, your tables
arrive fully populated — so treat this step as a quick pass to confirm
nothing looks off, rather than data entry.
...
- **Dropdown-driven columns** (e.g. IPCC coefficient descriptions in
  ...
  mistype a category.
- **Numeric cells turn red** if you type text or a negative number
  where a real, non-negative number is expected. The **Run herdr
  Model** button stays disabled — with a note listing which tables need
...
- At the bottom of each tab, a **"Next Step"** button takes you straight
  to the next relevant tab — on the last one, it becomes **"Ready?
  Click 'Run herdr Model'"**, which runs the calculation for you.
...
preference), use the **light/dark toggle** in the top-right corner of the
app — it applies to the whole app, including the data tables.

### Step 7 — Download your results

Click **Download Results** in the sidebar at any time after a
successful run to save the full results table as a CSV
